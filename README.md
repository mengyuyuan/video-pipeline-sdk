...
> 把「Remotion 口播视频」的制作从手工艺变成流水线：**九步流程 + 七件核验门 + 弹药仓 + 音画同步核验**。
>
> v0.2.1 · MIT · 2026-09
...
- **七件核验门（全可跑）**：帧检 / 转场可感知 / 查重 / 素材↔台词对位 / 五件套自检 / 排版网格 / 音频响度 —— `gates/selftest.ps1` 一键自检
- **音画同步核验**：互相关实测到 ms 级（`tools/check_sync.py`），交付前 |偏移| ≤ 15ms（实测可达 0.0ms）
- **弹药仓**：19 件效果件（含卡片系统 + 空间融合件）、字体、胶片 LUT、现成音效库、素材台账
...
- 质量不靠执行者自觉：参照物只作验收标尺；七件门 + 对照表 + 新机制配额硬卡
